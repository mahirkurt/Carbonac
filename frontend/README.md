...
- Frontend redirect URL’i `VITE_SITE_URL` (yoksa `window.location.origin`) üzerinden üretilir ve `/auth/callback` yoluna döner.
- Google ile girişten sonra `127.0.0.1` / `ERR_CONNECTION_REFUSED` görüyorsanız genelde Supabase Dashboard → Authentication → URL Configuration altında **Site URL** / **Redirect URLs** alanları production domain’inizi içermiyordur.
